...
func (c *client) get(path string, o options, kind string) error {
	...
	if err != nil {
		return fmt.Errorf("API is unavailable: %w", err)
	}
	...
	if err := json.Unmarshal(body, &payload); err != nil {
		return fmt.Errorf("API returned invalid data (%d)", res.StatusCode)
	}
	if res.StatusCode >= 400 {
		return apiError(payload, res.StatusCode)
	}
	if o.json {
...
func menuAction(c *client, selected int) error {
	...
	case 0:
		...
		return c.search(value, options{page: 1, limit: 10})
	case 1:
		value, err := prompt("Game slug or IGDB ID: ")
		if err != nil || value == "" {
			return errors.New("a game identifier is required")
		}
		return c.game(value, options{page: 1, limit: 10})
	case 2:
	...
	case 3:
		...
		if value == "" {
			return c.get("/series?limit=10&page=1", options{page: 1, limit: 10}, "series")
		}
		return c.get("/series/"+url.PathEscape(value), options{page: 1, limit: 10}, "series")
	case 4:
...
func setRawTerminal(raw bool) error {
	if !raw {
		return exec.Command("stty", "sane").Run()
	}
	return exec.Command("stty", "-icanon", "-echo", "min", "1", "time", "0").Run()
}
